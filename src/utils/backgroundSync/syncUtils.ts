  ...
  try {
    ...
    if (isOnline) {
      ...
      try {
        ...
        console.warn(
          "Failed to submit form while online, falling back to background sync",
          error,
        );
...
export const registerPeriodicSync = async (
  tag: string = "content-update",
  minInterval: number = 24 * 60 * 60,
): Promise<boolean> => {
  try {
      ...
      for (const entry of entries) {
        try {
          ...
          if (response.ok) {
            ...
            console.warn(
              `Sync failed for entry ${entry.id}: ${response.status}`,
            );
          }
        } catch (error) {
