...
const OptimizedImage: React.FC<OptimizedImageProps> = ({
  ...
  const generateSources = useCallback(() => {
    ...
    return {
      avif: supportsAVIF() ? baseUrl.replace(/\.(jpg|jpeg|png|webp)$/i, '.avif') + (queryString ? `?${queryString}` : '') : null,
      webp: supportsWebP() ? baseUrl.replace(/\.(jpg|jpeg|png)$/i, '.webp') + (queryString ? `?${queryString}` : '') : null,
        ...
        if (resourceEntries.length > 0) {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.debug('Image Performance:', {
              ...
              transferSize: entry.transferSize ? `${(entry.transferSize / 1024).toFixed(2)}KB` : 'unknown',
              decodedBodySize: entry.decodedBodySize ? `${(entry.decodedBodySize / 1024).toFixed(2)}KB` : 'unknown',
      ...
      {isError ? (
        ...
        <picture>
          ...
          <img
            ...
            style={{ 
              filter: !isLoaded && !isError && currentSrc ? 'blur(10px)' : 'none',
