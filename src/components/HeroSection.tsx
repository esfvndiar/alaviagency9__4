...
const HeroSection: React.FC = () => {
  ...
  useEffect(() => {
      ...
      if (isTypingStatic) {
        if (texts.staticText.length < fullStaticText.length) {
          ...
          if (animationQueueRef.current.length === 0) {
            setTexts((prevTexts) => ({
              ...prevTexts,
              staticText: fullStaticText.substring(
                0,
                prevTexts.staticText.length + 1,
              ),
            }));
          }
        } else {
        ...
        if (isDeleting) {
          ...
          if (texts.dynamicText.length > 0) {
            ...
            if (animationQueueRef.current.length === 0) {
              setTexts((prevTexts) => ({
                ...prevTexts,
                dynamicText: fullText.substring(
                  0,
                  prevTexts.dynamicText.length - 1,
                ),
              }));
            }
          } else {
    ...
    texts.staticText.length,
    texts.dynamicText.length,
    isTypingStatic,
    isDeleting,
    rotatingTexts,
  ]);
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div
        className="absolute bottom-10 left-1/2 transform -translate-x-1/2 flex flex-col items-center will-change-transform"
