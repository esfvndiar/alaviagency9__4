...
describe('AboutSection Component', () => {
  ...
  it('should render the main "About ALAVI" heading', async () => {
    ...
    expect(screen.getByRole('heading', { level: 2, name: /About ALAVI/i })).toBeInTheDocument();
  });

  it('should render the "Who We Are" subheading', async () => {
    renderAbout();
    // Directly find the subheading
    expect(screen.getByRole('heading', { level: 3, name: /Who We Are/i })).toBeInTheDocument();
  });

   it('should render the "Our Story" tag', async () => {
      renderAbout();
      // Directly find the tag
      expect(screen.getByText(/Our Story/i, { selector: 'span' })).toBeInTheDocument();
   });
   ...
   it('should render the AnimatedText content', async () => {
        ...
        expect(await within(animatedTextContainer).findByText(/^full-service$/i)).toBeInTheDocument();
        expect(await within(animatedTextContainer).findByText(/^IT$/i)).toBeInTheDocument(); // Use exact match regex
        expect(await within(animatedTextContainer).findByText(/^agency$/i)).toBeInTheDocument();
   });

   it('should render the stats section', async () => {
      renderAbout();
      const statsTexts = [/Clients/i, /150\+/i, /Projects/i, /200\+/i, /Years Experience/i, /10\+/i];
