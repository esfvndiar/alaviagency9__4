...
const UpdatePrompt = memo(
  ({ onUpdate, onLater }: { onUpdate: () => void; onLater: () => void }) => (
      ...
      <div className="flex items-start">
        ...
        <div className="ml-3 flex-1">
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            A new version of this website is available. Refresh to see the
            latest updates.
          </p>
...
const ServiceWorkerUpdater: React.FC = () => {
  ...
  const updateFoundHandlersRef = useRef<
    WeakMap<ServiceWorkerRegistration, EventListener>
  >(new WeakMap());
  const stateChangeHandlersRef = useRef<WeakMap<ServiceWorker, EventListener>>(
    new WeakMap(),
  );
        ...
        if (registration) {
          ...
          intervalRef.current = window.setInterval(async () => {
            try {
              ...
              console.error(
                "ServiceWorkerUpdater: Error checking for updates",
                {
      ...
      navigator.serviceWorker
        ...
        .then((registration) => {
          if (registration) {
            ...
            const updateFoundHandler =
              currentUpdateFoundHandlers.get(registration);
            ...
            [registration.installing, registration.waiting, registration.active]
              ...
              .forEach((worker) => {
                const stateChangeHandler =
                  currentStateChangeHandlers.get(worker);
  ...
  return (
    <UpdatePrompt onUpdate={updateServiceWorker} onLater={hideUpdatePrompt} />
  );
};
