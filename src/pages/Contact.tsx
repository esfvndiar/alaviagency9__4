...
import {
  ...
  registerSync,
} from "../utils/backgroundSync/syncUtils";
...
const Contact: React.FC = () => {
  ...
  const [submitStatus, setSubmitStatus] = useState<
    "idle" | "success" | "error" | "offline-queued"
  >("idle");
  const [errorMessage, setErrorMessage] = useState("");
  const [isBackgroundSyncAvailable, setIsBackgroundSyncAvailable] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const handleOnline = () => {
      if (submitStatus === "offline-queued") {
        ...
        setErrorMessage(
          "You are back online. Your message will be sent automatically.",
        );
      ...
      if (!navigator.onLine) {
        ...
        if (isBackgroundSyncAvailable) {
          ...
          if (success) {
            ...
            throw new Error(
              "Failed to queue form submission for background sync",
            );
          }
        } else {
                  ...
                  <div className="bg-white dark:bg-zinc-800 rounded-2xl shadow-xl p-8 relative overflow-hidden">
                    ...
                    {submitStatus === "success" && (
                      <div className="absolute inset-0 bg-white dark:bg-zinc-800 flex flex-col items-center justify-center p-8 animate-fade-in z-10">
                        ...
                        <h3 className="text-2xl font-medium mb-2 text-zinc-900 dark:text-white">
                          Message Sent!
                        </h3>
                    ...
                    {submitStatus === "offline-queued" && (
                      <div className="absolute inset-0 bg-white dark:bg-zinc-800 flex flex-col items-center justify-center p-8 animate-fade-in z-10">
                        ...
                        <p className="text-zinc-600 dark:text-zinc-300 text-center mb-6">
                          Your message has been saved and will be sent
                ...
                <ScrollReveal delay={200}>
                  <div className="bg-white dark:bg-zinc-800 rounded-2xl shadow-xl p-8 relative overflow-hidden">
                    ...
                    <p className="text-zinc-600 dark:text-zinc-300 mb-6">
                      ...
                      confirm your appointment via email.
                    </p>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-7 gap-1 p-4">
                        ...
                        {[28, 29, 30, 31].map((day) => (
                          <button
                            key={`prev-${day}`}
                            className="h-10 w-full rounded-full text-zinc-400 dark:text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-600 transition-colors text-sm"
                          >
                            {day}
                          </button>
                        ))}
                    ...
                    <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-4">
                      Note: This is a placeholder calendar. In a production
                      environment, you would integrate with a calendar service
                      like Google Calendar, Calendly, or a custom solution.
                    </p>
                  </div>
                </ScrollReveal>
              </div>
